...
async function getFoodList() {
    ...
    const response = await fetch(`http://localhost:3000/${currentWeek}`);
...
function generateFoodList(foodList) {
    ...
    if (foodList && foodList.length > 0) {
        ...
        for (let i = 0; i < foodList.length; i++) {
            ...
            foodFichesTime.innerText = `Préparation : ${fiches.time} Minutes`;
            foodFichesTime.classList = "foodFichesTime";
            //Balises Type
            const foodFichesType = document.createElement("p");
            foodFichesType.innerText = `Idéal pour : ${fiches.type}`;
